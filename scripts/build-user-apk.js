...
console.log('📱 Configuring for 7 STAR INVEST (User App)...');
...
buildGradle = buildGradle.replace(/applicationId "com\.star7\.(admin|invest)"/g, 'applicationId "com.star7.invest"');
...
<resources>
    <string name="app_name">7 STAR INVEST</string>
    <string name="title_activity_main">7 STAR INVEST</string>
    <string name="package_name">com.star7.invest</string>
    <string name="custom_url_scheme">com.star7.invest</string>
</resources>
`;
...
console.log('⚙️ Building 7 STAR INVEST Release APK...');
...
const targetApk = path.join(rootDir, 'public/7star-invest.apk');

if (fs.existsSync(outputApk)) {
  fs.copyFileSync(outputApk, targetApk);
  const stats = fs.statSync(targetApk);
  console.log(`✅ SUCCESS! User APK created at public/7star-invest.apk (${(stats.size / 1024 / 1024).toFixed(2)} MB)`);
} else {
